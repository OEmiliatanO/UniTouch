...
for n in range(population):
    ...
    with torch.no_grad():
        preds = []
        all_labels = []
        for batch in tqdm(touch_training_dataloader, desc=f"Evaluating model {n+1}/{population}"):
        ...
        acc = (preds == torch.tensor(all_labels)[touch_training_dataset.indices]).float().mean().item()
        accs.append(acc)

best_indices = np.argsort(accs)[-1]
best_omnimodel = transfer(touch_model, imagebined_model, modalities=[ModalityType.TOUCH], seed=best_indices)

with torch.no_grad():
    preds = []
    for batch in tqdm(touch_testing_dataloader, desc=f"Evaluating best model on test set"):
        touch_data, labels = batch
        touch_data = touch_data.to(device)
        outputs = best_omnimodel({ModalityType.TOUCH: touch_data}) # Shape: [batch_size, 1024]
        touch_features = outputs[ModalityType.TOUCH] # Shape: [batch_size, 1024]
        batch_preds = (touch_features @ text_features.T).argmax(dim=-1) # Shape: [batch_size]
        preds.append(batch_preds.cpu())
    preds = torch.cat(preds, dim=0) # Shape: [num_samples]
    acc = (preds == torch.tensor(all_labels)[touch_testing_dataset.indices]).float().mean().item()
